...
conforme a relação exposta na~\Cref{tbl:relação-chaves-campos-json}.
...
\todo{%
    Em algum ponto, esclarecer que as duas estratégias foram mantidas pois,
    apesar do TJ-Rj possuir uma API JSON, outros TJs podem não ter, então a
    implementação existente pode ser útil.
}
...
as~\Cref{lst:requisição-processos-síncrona,lst:requisição-processos-assíncrona}
...
para a coluna \textit{``state''} descritos na~\Cref{tbl:valores-coluna-state}.
